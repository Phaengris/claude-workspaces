...
| 2 | Line format | Two-part, one line per step: `  <project>: checking out (branch <base>)… ` / `  <project>: setup: <substituted command>… ` printed WITHOUT newline before the step; `ok (2.1s)\n` or `failed\n` appended when it ends. A failed step's error then propagates exactly as today. |
