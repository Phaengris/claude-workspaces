...
// browseConfig: app has a substitutable port, noport has none — the two halves
// of the browse_port contract.
const browseConfig = `values:
...
// browseRoot builds a root holding browseConfig plus one allocation at
// <root>/A-1_x, and empties PATH so neither git nor xdg-open can be found:
// nothing is checked out (checked-out detection degrades to "no"), and browse
// must take the print-the-URL path rather than opening a browser on the
// machine running the tests.
func browseRoot(t *testing.T) string {
	t.Helper()
	root := fixtureRoot(t, map[string]string{"config.yml": browseConfig})
...
// usage error 2 like any wrong arg shape, and a configured project whose
// browse_port resolves empty is a plain 1 — the identifier was fine, the
// config has nothing to browse. An explicit configured project with a port
// needs NO checkout: 0.
...
func TestBrowsePrintsURLWithoutOpener(t *testing.T) {
	...
	if got, want := out.String(), "http://localhost:5000\n"; got != want {
