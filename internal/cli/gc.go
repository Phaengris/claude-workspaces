...
//     CHECKED-OUT project (ProjectStates' subset rule, the same one status
//     renders): a pid file that exists but does not name a live process is
//     removed, one `reaped stale pid file <ws>/<project:daemon>` line each.
//     "Not live" includes a CORRUPT pid file — DaemonState already treats
//     corrupt as not running (the decided Liveness row), and a record nothing
//     can act on is exactly the garbage this pass exists to collect. A LIVE
//     daemon's record is untouchable.
//
...
//     destroy command runs, force=false) when ALL of:
//
...
//     only on evidence, so it SURVIVES (use `workspace destroy` to say
//     "I mean it" about an empty workspace);
...
// runs to completion, and the joined error exits 1. The summary line is
// always printed — `gc: N released, M reaped, K destroyed`, counting only
// successes — EXCEPT when there was truly nothing to do and nothing failed:
// then the single line `nothing to collect` (exit 0) says so instead.
func newGCCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Pass 2 — reap stale pid files of checked-out projects.
			for _, ws := range survivors {
				for _, st := range wsp.ProjectStates(cfg, ws) {
					for _, d := range wsp.DaemonsOf(cfg, st.Name) {
			...
			// Pass 3 — destroy fully merged, daemonless, tool-created
			// workspaces that actually contain something.
			if destroyDirs {
				for _, ws := range survivors {
					if ws.Alloc.Adopted {
	...
	cmd.Flags().BoolVarP(&destroyDirs, "destroy-dirs", "d", false,
		"also destroy tool-created workspaces whose branches are fully merged and which run no daemon")
	return cmd
}
